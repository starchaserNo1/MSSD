...
public class Tree_clusterSet3 {
	...
	public static ArrayList<ArrayList<Node>> getClusterSet(ArrayList<Node> net){
		...
		ArrayList<ArrayList<Node>> clusterBujiSet=new ArrayList<ArrayList<Node>>();
		for(int i=0;i<net.size();i++){
			if(net.get(i).parents.size()!=0){
				...
				if(MkNetCollOne.IsAbleAdd(clusterSet, cluster)&&MkNetCollOne.IsAbleAdd(clusterBujiSet, cluster)){
					clusterSet.add(cluster);
					clusterBujiSet.add(clusterBuji);
				}
			}
		}
		return clusterSet;
